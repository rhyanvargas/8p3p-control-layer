...
import { handleTrajectoryQueryCore } from './trajectory-handler-core.js';
...
export async function handleTrajectoryQuery(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<unknown> {
  const result = await handleTrajectoryQueryCore(request.query as Record<string, unknown>);
  reply.status(result.statusCode);
  return result.body;
}
