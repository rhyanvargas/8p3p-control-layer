/**
 * State Query Handler
 * Handles GET /v1/state and GET /v1/state/list for Inspection API
 */

import type { FastifyRequest, FastifyReply } from 'fastify';
import type { LearnerState, RejectionReason } from '../shared/types.js';
...
/**
 * Validate GET /v1/state query parameters (single learner)
 */
function validateStateParams(params: Record<string, unknown>): {
  ...
  if (params.version !== undefined && params.version !== '') {
    const versionNum =
      typeof params.version === 'string' ? parseInt(params.version, 10) : params.version;
...
/**
 * Validate GET /v1/state/list query parameters
 */
...
/**
 * Handle GET /state request (single learner state)
 */
...
/**
 * Handle GET /state/list request (list learners)
 */
export async function handleStateListQuery(
  ...
  const { learners, nextCursor } = listLearners(
    validation.org_id!,
    validation.limit!,
    validation.cursor
  );
