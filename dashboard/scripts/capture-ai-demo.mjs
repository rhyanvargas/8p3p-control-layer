/**
 * Capture AI educator-explanation demo screenshots.
...
async function main() {
  ...
  try {
    ...
    // 15 — Overview AI insight panels + narrative summaries in recent decisions
    await page.goto(`${BASE}/`, { waitUntil: 'domcontentloaded' });
    await page.getByRole('heading', { name: 'Overview', exact: true }).waitFor({ timeout: 30_000 });
    await page.getByText(/AI educator insights|Act now|Also watching/i).first().waitFor({ timeout: 30_000 });
    await page.getByText(/less confident|reinforcement|ready to move|check-in|noisy/i).first().waitFor({ timeout: 30_000 });
    await shot(page, '15-ai-overview-insights.png');

    // Scroll recent decisions into view for narrative column
    const recent = page.getByRole('heading', { name: 'Recent decisions' });
    if (await recent.count()) {
      await recent.scrollIntoViewIfNeeded();
      await page.waitForTimeout(400);
      await shot(page, '15b-ai-overview-recent-summaries.png');
    }

    // 16 — Attention review sheet with AI explanation
    ...
    // Open first queue row (not the Approve button)
    const row = page.locator('[data-slot="table-row"]').filter({ hasText: /Intervene|Pause/i }).first();
    await row.click();
    await page.getByText('AI explanation').waitFor({ timeout: 15_000 });
    await page.getByText(/less confident|noisy|reinforcement|check-in/i).first().waitFor({ timeout: 15_000 });
    await shot(page, '16-ai-attention-explanation.png');

    // 17 — Decision peek with AI explanation
    ...
    // 18 — Decision trace with AI explanation
    await page.getByRole('link', { name: /Open trace/i }).or(page.getByRole('button', { name: /Open trace/i })).first().click();
    await page.getByRole('heading', { name: /Decision trace/i }).waitFor({ timeout: 30_000 });
    await page.getByText('AI explanation').waitFor({ timeout: 15_000 });
    await shot(page, '18-ai-decision-trace.png');

    // 19 — Learner overview with AI summaries in recent decisions
    ...
    await page.getByText(/less confident|reinforcement|ready to move|check-in|Needs stronger|Confidence and mastery/i).first().waitFor({ timeout: 30_000 });
