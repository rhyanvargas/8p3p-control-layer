...
1. Update top-level `security` from `[]` to `[ApiKeyAuth: []]` and add a note in `info.description` that auth is env-dependent (disabled when `API_KEY` unset)
2. Add 401 response to `POST /v1/signals`, `GET /v1/signals`, `GET /v1/decisions`:
