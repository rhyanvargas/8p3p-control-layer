...
To preload demo data (same host, e.g. for training): run `npm run seed:demo` against the server with `--api-key <your_key>` and `--org <org_id>` (or use the org assigned to your key). See [Demo Walkthrough](demo-walkthrough.md).
