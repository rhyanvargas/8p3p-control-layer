...
- **Interim auth decision (resolved 2026-06-29):** **Dual access codes** — educator passphrase vs compliance/admin passphrase — until Cognito/SSO (Phase 2). Spec home: [`dashboard-passphrase-gate.md`](../specs/dashboard-passphrase-gate.md) § Dual access codes.
...
Normative home: [`docs/specs/dashboard-design-requirements.md`](../specs/dashboard-design-requirements.md) §2.2 D5 — Persona surfaces.
...
| Dual-passphrase auth | **Spec'd; not implemented** | Dual-code env vars + persona cookie + route allowlists in [`dashboard-passphrase-gate.md`](../specs/dashboard-passphrase-gate.md) § Dual access codes; runtime still single `DASHBOARD_ACCESS_CODE` until PE-001 |

### Capability vs infrastructure tension (Check 2 — both sides cited)

These claims **conflict** until persona middleware ships (PE-001–PE-006) or interim mitigations are enforced:
...
**Reconciliation (interim pilot):** Distribute **two passphrases** (educator vs compliance) and follow the **two-path demo script** ([`springs-pilot-demo.md`](../guides/playbooks/springs-pilot-demo.md), [`organic-educator-wave-zoom.md`](../guides/playbooks/organic-educator-wave-zoom.md)) until [`dashboard-persona-enforcement.plan.md`](../../.cursor/plans/dashboard-persona-enforcement.plan.md) PE-001–PE-006 enforce route/nav allowlists. Long-term auth owner: Phase 2 Cognito replaces codes, **not** D5 IA rules ([`dashboard-passphrase-gate.md`](../specs/dashboard-passphrase-gate.md)).

---

## 4. Role × feature × infrastructure map (SSoT for §D5)

Document this table in §D5 ([`dashboard-design-requirements.md`](../specs/dashboard-design-requirements.md) §2.2) and use it for Zoom/demo routing:
...
| §D5 persona surfaces normative in design spec | **Shipped (doc)** | [`dashboard-design-requirements.md`](../specs/dashboard-design-requirements.md) §2.2 D5 (ceo doc plan TASK-016) |
...
|---|--------|----------|----------|--------|
| E1 | Document §D5 persona surfaces + role × feature table | P0 (doc) | `docs/specs/dashboard-design-requirements.md` | **Done** (TASK-016) |
| E2 | Dual-passphrase spec (`DASHBOARD_ACCESS_CODE_EDUCATOR` / `_COMPLIANCE`) | P0 (doc) | `docs/specs/dashboard-passphrase-gate.md` | **Done** (TASK-017) |
| E3 | Create persona **implementation** plan (PE-001–PE-008) | P0 | `.cursor/plans/dashboard-persona-enforcement.plan.md` | **Done** (TASK-021) |
| E4 | Two-path demo + Zoom runbook | P0 (GTM) | `springs-pilot-demo.md`, `organic-educator-wave-zoom.md` | **Done** (TASK-006, TASK-010) |
| E5 | CEO report + roadmap ledger (this file, roadmap §D5) | P0 (doc) | this file, `docs/foundation/roadmap.md` | **Done** (TASK-001/002) |
| E6 | Policy builder scaffold (compliance-only writes MVP-1) | P1 | `educator-policy-builder.md`, companion plan | **Done** (TASK-007–009) |
