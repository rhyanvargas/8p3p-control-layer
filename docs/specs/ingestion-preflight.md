...
- **Pilot readiness gate (already landed in pilot-readiness-definition.md).** The integration-gate row in `internal-docs/pilot-operations/pilot-readiness-definition.md` § Integration ("Preflight passes on raw sample — BLOCKING") and the customer-readiness row ("Raw sample payload preflight clean") together codify the gate. No further doc-touch task is required when the endpoint ships; the gate is already in force and the endpoint must satisfy it. The `/plan-impl` task list MUST verify both rows still match the response shape defined here (`forbidden_pii`, `forbidden_semantic_after_mapping`, `verdict`) before merge.
