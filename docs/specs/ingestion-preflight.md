...
- Batch preflight (multi-payload one-request) — a single sample is the documented CS workflow per [`docs/guides/operators/pilot-readiness-gates.md`](../guides/operators/pilot-readiness-gates.md).
...
| Capability | Used By |
|-----------|---------|
| `POST /v1/admin/ingestion/preflight` | [`docs/guides/operators/pilot-readiness-gates.md`](../guides/operators/pilot-readiness-gates.md) § Integration gate; CS/Solutions during pilot intake; future `scripts/preflight-sample.mjs` CLI |
...
| INGEST-PREFLIGHT-004 | Semantic key resolved by existing mapping | mapping for `(springs, canvas-lms)` defines `masteryScore = submission.score / submission.total`; `{ org_id: 'springs', source_system: 'canvas-lms', payload: { submission: { score: 85, total: 100 } } }` | 200; `forbidden_semantic_raw: [{ key: 'score', path: 'payload.submission.score', ... }]`, `forbidden_semantic_after_mapping: [{ key: 'score', path: 'payload.submission.score' }]` (additive normalization retains raw `score`; see test strategy note), `verdict: "semantic_blocking"` |
...
| FORBIDDEN-KEYS-SPLIT-001 | Exports `FORBIDDEN_PII_KEYS` with expected membership | import from `src/ingestion/forbidden-keys.ts` | Set contains all 28 PII keys listed in § Requirements; contains **zero** semantic keys |
...
- **Pilot readiness gate.** The integration-gate row in [`docs/guides/operators/pilot-readiness-gates.md`](../guides/operators/pilot-readiness-gates.md) § Integration ("Preflight passes on raw sample — BLOCKING") and the customer-readiness row ("Raw sample payload preflight clean") codify the gate. Before merge of the endpoint implementation, verify both rows match the response shape defined here (`forbidden_pii`, `forbidden_semantic_after_mapping`, `verdict`).
